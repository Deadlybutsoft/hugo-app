...
export default function AIAssistantUI() {
  ...
  function sendMessage(convId, content) {
    ...
    // Add the empty assistant message to the conversation immediately (optional, or wait for first chunk)
    // Here we'll wait for the first chunk or just set thinking to true.
    // Actually, let's add it when we start receiving data or just keep thinking state.
    // But to stream into it, we usually need it to exist.
    // Let's keep isThinking=true until the stream starts, then append the message.

    // Prepare messages for API
    const conversation = conversations.find(c => c.id === convId)
    const previousMessages = conversation?.messages || []
    const messagesToSend = [...previousMessages, userMsg].map(m => ({ role: m.role, content: m.content }))
    ...
    fetch("/api/chat", {
      ...
      .then(async (response) => {
        ...
        // Add the assistant message to state once we have a response
        setConversations((prev) =>
          prev.map((c) => {
        ...
        setIsThinking(false) // Stop showing the "thinking" indicator, show the streaming message instead
