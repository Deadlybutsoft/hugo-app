...
export default function ThemeToggle({ theme: propTheme, setTheme: propSetTheme }) {
    const { theme: systemTheme, setTheme: systemSetTheme } = useTheme();
    ...
    // If propTheme is defined, use it. Otherwise use systemTheme.
    const currentTheme = propTheme !== undefined ? propTheme : systemTheme;

    // User Request: "when the page will be dark mode the icon will show light mode"
    // CSS Logic: Unchecked = Sun (Light Icon), Checked = Moon (Dark Icon)
    // Therefore:
    // Dark Mode => Show Sun => Unchecked
    // Light Mode => Show Moon => Checked
    const isChecked = currentTheme === 'light';

    const handleToggle = (e) => {
        // Logic:
        // If currently Unchecked (Sun/Dark), user clicks -> Checked (Moon).
        // We want to switch TO Light mode (so we can show the Moon as "Active" or just per the cycle).
        // Wait, if Dark -> Sun. Click Sun -> Go Light.
        // If Light -> Moon. Click Moon -> Go Dark.
